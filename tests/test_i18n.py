...
class TranslationContractTests(SimpleTestCase):
    ...
    def test_localized_messages_are_loaded_from_package_catalog(self):
        with override("ru"):
            self.assertEqual(
                self._protected_error_message(),
                gettext("cannot be deleted because related %(object_name)s exist")
                % {"object_name": gettext("related data")},
            )
            self.assertEqual(
                self._all_query_description(),
                gettext("Retrieve all results at once"),
            )
